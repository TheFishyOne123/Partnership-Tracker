  ...
  const exportAsCsv = () => {
    if (!search) {
      ...
      if (searchResults.length <= 0) {
        ...
        const csvData = searchResults.map(
          ({ _id, __v, ...csvData }) => csvData
        );
        exportFromJSON({
          data: csvData,
  ...
  const exportAsXls = () => {
    if (!search) {
      ...
      if (searchResults.length <= 0) {
        ...
        const xlsData = searchResults.map(
          ({ _id, __v, ...xlsData }) => xlsData
        );
        exportFromJSON({
          data: xlsData,
          fileName: "SearchReport",
          ...
          <thead>
            <tr className="text-lg tst:text-sm" key="Head">
              ...
              <th
                ...
                title="Email Of Partnership Manager Or Owner "
              >
                Email
              </th>
              <th
